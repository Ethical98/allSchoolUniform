...
export const addToCart = (id, index, qty) => async (dispatch, getState) => {
    try {
        ...
        if (userInfo) {
            ...
            const addItem = {
                ...
                maxOrderQty: data.size[index].maxOrderQty,
    ...
    localStorage.setItem('cartItems', encryptData(JSON.stringify(getState().cart.cartItems), salt));
};
...
export const mergeCartWithDatabase = () => async (dispatch, getState) => {
    try {
        ...
        if (userInfo) {
            ...
            localStorage.setItem('cartItems', encryptData(JSON.stringify(getState().cart.cartItems), salt));
            localStorage.setItem('cartSuccess', JSON.stringify(getState().cart.cartSuccess));
        }
    } catch (error) {
        dispatch({
            type: CART_ITEM_FAIL,
