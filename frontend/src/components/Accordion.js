...
const Accordion = () => {
    ...
    const [category, setCategory] = useState(params.category ? params.category.split(',') : []);

    const [season, setSeason] = useState(params.season ? params.season.split(',') : []);
    const [standard, setStandard] = useState(params.class ? params.class.split(',') : []);
    ...
    useEffect(() => {
        ...
        if (category.length > 0 && season.length > 0 && standard.length > 0) {
            if (!pageNumber) {
                if (school) {
                    history.push(`/products/schools/${school}?season=${season}&category=${category}&class=${standard}`);
                } else if (keyword) {
                    history.push(`/products?search=${keyword}&season=${season}&category=${category}&class=${standard}`);
                } else {
                    history.push(`/products?season=${season}&category=${category}&class=${standard}`);
                }
            }
        } else if (category.length > 0 && season.length > 0) {
            if (!pageNumber) {
                if (school) {
                    history.push(`/products/schools/${school}?season=${season}&category=${category}`);
                } else if (keyword) {
                    history.push(`/products?search=${keyword}&season=${season}&category=${category}`);
                } else {
            ...
            if (!pageNumber) {
                if (school) {
                    history.push(`/products/schools/${school}?category=${category}&class=${standard}`);
                } else if (keyword) {
                    history.push(`/products?search=${keyword}&category=${category}&class=${standard}`);
                } else {
            ...
            if (!pageNumber) {
                if (school) {
                    history.push(`/products/schools/${school}?season=${season}&class=${standard}`);
                } else if (keyword) {
                    history.push(`/products?search=${keyword}&season=${season}&class=${standard}`);
                } else {
    ...
    }, [category, season, dispatch, standard, pageNumber, history, school, keyword]);
                    ...
                    <div id="ProductCategory" className="collapse show">
                        <Form className="m-3">
                            <Form.Check
                                ...
                                value="Boys"
                                label="Boys"
                                onChange={event => categoryChange(event.target.value, event.target.checked)}
                            ></Form.Check>
                            <Form.Check
                    ...
                    <div id="SeasonalClothing" className="collapse show">
                        <Form className="m-3">
                            <Form.Check
                                ...
                                value="Winter"
                                label="Winter"
                                onChange={event => seasonChange(event.target.value, event.target.checked)}
                            />
                            <Form.Check
                                        ...
                                        x =>
                                            x.isActive === true && (
                                                <Form.Check
                                                    ...
                                                    type="checkbox"
                                                    label={x.class}
                                                    onChange={e => standardChange(x.class, e.target.checked)}
                                                />
                                            )
                                    )}
                            </ListGroup.Item>
