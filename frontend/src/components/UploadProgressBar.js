...
const UploadProgressBar = ({
  ...
  const { progress: schoolImageUploadProgress, url: schoolImageUrl } =
    schoolImageUpload;
  const typeImageUpload = useSelector((state) => state.typeImageUpload);
  const {
    progress: typeImageUploadProgress,
    imageOneUrl,
    imageTwoUrl,
    imageThreeUrl,
  } = typeImageUpload;
  ...
  useEffect(() => {
    if (product) dispatch(uploadProductImage(file));
      ...
      dispatch(
        uploadTypeImage(file, typeImageOne, typeImageTwo, typeImageThree)
      );
  }, [
    file,
    product,
