...
const OrderEditScreen = ({ history, match, location }) => {
    ...
    const SaveChangesButton = () => (
        <Button className="my-3 float-end" variant="primary" onClick={saveChangesHandler}>
            Save Changes
        </Button>
    );
            ...
            >
                ...
                {loadingProducts ? (
                    ...
                    <>
                        ...
                        />
                        <Paginate orderEdit="true" pages={pages} page={page} isAdmin="true" orderId={orderId} />
                    </>
                )}
            </DialogBox>
            <DialogBox handleClose={closeTrackingHandle} show={showTracking} title="Edit Tracking Details">
                <Row>
                    <Form>
                        {errorConfirm ? (
