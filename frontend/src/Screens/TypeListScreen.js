...
const TypeListScreen = ({ history }) => {
    ...
    const columns = [
        ...
        { title: 'Type', field: 'type' },
        {
            title: 'Image',
            field: 'image',
            render: item => <Image src={item.image} alt={item.type} style={{ width: '5vw' }} fluid rounded />
        },
        {
            title: 'Size Guide',
            field: 'sizeGuide',
            render: item => <Image src={item.sizeGuide} alt={item.type} style={{ width: '5vw' }} fluid rounded />
        },
        {
            title: 'Size Chart',
            field: 'sizeChart',
            render: item => <Image src={item.sizeChart} alt={item.type} style={{ width: '5vw' }} fluid rounded />
        }
    ];
    ...
    return (
        <AdminPageLayout>
            <Meta title={'Product Type List - AllSchoolUniform'} description={'Product List Page'} />
                        ...
                        actions={[
                            {
                                icon: 'edit',
                                tooltip: 'Edit',
                                onClick: (event, rowData) => history.push(`/admin/type/${rowData._id}/edit`)
                            }
                        ]}
                    />
                </>
